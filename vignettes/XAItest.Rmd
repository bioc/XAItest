...
```{r addSimThresh}
set.seed(1)
# Add the simThresh variable to achieve a p-value = 0.0001 between A and B
df_st <- addSimThresh(df1, pval_target = 0.0001, tolerance = 0.00001)
df_st[c(1:2,50:51),c(1:3,(ncol(df_st)-2):ncol(df_st))]
```

We verify that the p-value is indeed close to 0.0001
...
Now we will apply the same approach to a regression scenario using `df6`, which contains the rising sinusoid relationship pattern.

```{r addSimThresh regress}
set.seed(1)
# Add the simThresh variable to achieve a p-value = 0.00001 for the correlation with `y`
df_st <- addSimThresh(df6, pval_target = 0.00001, tolerance = 0.000001)
...
The feature `unif_noise01` is indeed the feature used to build the rising sinusoid `y` feature.
